...
export default function NewTopicPage() {
    ...
    const { t } = useTranslation("Forum.newTopic");
    ...
    const toolbarButtons = [
        { icon: <Bold className="w-4 h-4" />, label: t("markdownHelp").split("**")[1] || "Bold", prefix: "**", suffix: "**" },
        { icon: <Italic className="w-4 h-4" />, label: t("markdownHelp").split("*")[1] || "Italic", prefix: "*", suffix: "*" },
        { icon: <Code className="w-4 h-4" />, label: "Code", prefix: "`", suffix: "`" },
        { icon: <Link2 className="w-4 h-4" />, label: "Link", prefix: "[", suffix: "](url)" },
        { icon: <List className="w-4 h-4" />, label: "List", prefix: "\n- ", suffix: "" },
        { icon: <ListOrdered className="w-4 h-4" />, label: "Ordered List", prefix: "\n1. ", suffix: "" },
        { icon: <Quote className="w-4 h-4" />, label: "Quote", prefix: "\n> ", suffix: "" },
        { icon: <Heading1 className="w-4 h-4" />, label: "H1", prefix: "\n# ", suffix: "" },
        { icon: <Heading2 className="w-4 h-4" />, label: "H2", prefix: "\n## ", suffix: "" },
    ];
    ...
    useEffect(() => {
        ...
        fetchForumCategories()
            ...
            .catch(() => {
                if (!active) return;
                setLoadError(t("noCategoriesDefined")); // Or generalized error
            })
            .finally(() => {
    ...
    const handleSubmit = async () => {
        ...
        if (!token) {
            toast.error(t("errors.sessionMissing"));
            ...
            <div className="min-h-screen px-4 py-8 md:px-6 lg:px-8">
                <div className="max-w-4xl mx-auto">
                    ...
                    >
                        ...
                        <span className="text-[var(--text-secondary)]">{t("breadcrumb")}</span>
                    </motion.div>
                    ...
                    >
                        ...
                        <div className="rounded-xl bg-[rgba(15,31,54,0.6)] border border-[rgba(72,213,255,0.1)] p-5">
                            ...
                            {loadingCategories ? (
                                ...
                                <p className="text-sm text-[var(--text-muted)]">{t("noCategoriesDefined")}</p>
                            ) : (
                                            ...
                                            >
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-xs text-[var(--text-muted)] line-clamp-1">
                                                        {cat.description || (meta.description ? tRoot(meta.description) : "")}
                                                    </p>
                                                </div>
                                            </button>
                                        );
                            ...
                            {showPreview ? (
                                <div className="min-h-[200px] p-4 rounded-xl bg-[rgba(0,0,0,0.2)] text-[var(--text-secondary)]">
                                    {content ? (
                                        ...
                                        <p className="text-[var(--text-muted)] italic">{t("noPreview")}</p>
                                    )}
                                </div>
                            ) : (
                                <textarea
                        ...
                        <div className="rounded-xl bg-[rgba(15,31,54,0.6)] border border-[rgba(72,213,255,0.1)] p-5">
                            ...
                            <p className="text-xs text-[var(--text-muted)] mt-2">
                                {t("maxTags").replace("5", tags.length + "/5")}
                            </p>
                        </div>
                    ...
                    >
                        ...
                        <ul className="space-y-2 text-sm text-[var(--text-muted)]">
                            {["0", "1", "2", "3", "4"].map((i) => (
                                <li key={i}>• {t(`guidelines.${i}` as any)}</li>
                            ))}
                        </ul>
                    </motion.div>
                </div>
