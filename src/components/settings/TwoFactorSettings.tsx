...
export default function TwoFactorSettings({ token }: TwoFactorSettingsProps) {
    const { t } = useTranslation("TwoFactorSettings");
    ...
    const handleSetup = async () => {
        ...
        try {
            ...
            if (data.success && data.qrCode) {
                ...
                toast.error(data.message || t("toasts.setupFailed"));
            }
        } catch (err) {
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                toast.error(data.message || t("toasts.verifyFailed"));
            }
        } catch (err) {
            toast.error(t("toasts.genericError"));
        } finally {
    ...
    const handleDisable = async () => {
        ...
        if (!disablePassword) {
            toast.error(t("toasts.passwordRequired"));
            return;
        }
        setSubmitting(true);
        try {
            ...
            if (data.success) {
                ...
                toast.error(data.message || t("toasts.disableFailed"));
            }
        } catch (err) {
            toast.error(t("toasts.genericError"));
        } finally {
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-[var(--text-muted)]">
                        {enabled ? t("status.activeDescription") : t("status.inactiveDescription")}
                    </p>
                </div>
            </div>
            ...
            <div className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium ${enabled
                ...
                {enabled ? (
                    <>
                        ...
                        {t("status.badgeActive")}
                    </>
                ) : (
                    <>
                        <AlertCircle className="h-4 w-4" />
                        {t("status.badgeInactive")}
                    </>
                )}
            </div>
            ...
            {setupMode && qrCode && (
                <div className="space-y-4 rounded-xl border border-[rgba(72,213,255,0.15)] bg-[rgba(15,31,54,0.5)] p-6">
                    <h4 className="font-medium text-white">{t("setup.title")}</h4>

                    <div className="flex flex-col items-center gap-4">
                        {/* QR Code */}
                        <div className="rounded-xl bg-white p-4">
                            <Image src={qrCode} alt={t("setup.qrAlt")} width={200} height={200} />
                        </div>
                        ...
                        {secret && (
                            <div className="w-full space-y-2">
                                <p className="text-xs text-[var(--text-muted)]">{t("setup.manualEntry")}</p>
                        ...
                        {/* Verification Input */}
                        <div className="w-full space-y-2">
                            <label className="text-sm text-[var(--text-muted)]">{t("setup.codeLabel")}</label>
                            <input
                                type="text"
                                maxLength={6}
                                value={verifyCode}
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : t("setup.confirm")}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4 rounded-xl border border-[rgba(255,100,100,0.15)] bg-[rgba(255,50,50,0.05)] p-6">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-sm text-[var(--text-muted)]">{t("disable.codeLabel")}</label>
                            <input
                                type="text"
                                maxLength={6}
                                value={disableCode}
                        ...
                        <div className="space-y-2">
                            <label className="text-sm text-[var(--text-muted)]">{t("disable.passwordLabel")}</label>
                            <input
                                type="password"
                        ...
                        >
                            {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : t("disable.button")}
                        </button>
                    </div>
                </div>
            )}
            ...
            <div className="rounded-xl bg-[rgba(30,184,255,0.05)] border border-[rgba(30,184,255,0.1)] p-4">
                <p className="text-sm text-[var(--text-muted)]">
                    <strong className="text-white">{t("info.lead")}</strong> {t("info.description")}
                </p>
            </div>
        </div>
    );
