...
import type { Project, ProjectPlatform, ProjectScreenshot, ProjectFeature, ProjectLink } from "@/types/project";
import {
    ArrowLeft,
    Trash2,
    Plus,
...
export default function ProjectEditForm({ project, isNew }: Props) {
    ...
    const [initialState] = useState<string>(() => {
        ...
        // For the purpose of initial state, we need to consider the initial logoUrl too
        // But referencing 'logoPreview' state here is tricky because it's initialized separately.
        // Instead, we pass the raw data's logoUrl to the normalization logic roughly.
        // Actually, let's just use the logic directly on the data we have.
    ...
    const handleSave = async () => {
        ...
        if (editData.descriptionsByLocale) {
            Object.entries(editData.descriptionsByLocale).forEach(([loc, value]) => {
                if (value?.trim()) cleanedDescriptions[loc] = value;
            });
        }
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                    ...
                                                    <div className="space-y-1.5">
                                                        ...
                                                        <select
                                                            ...
                                                            onChange={e => setEditData(prev => ({ ...prev, type: e.target.value as Project["type"] }))}
                                                            className="w-full h-10 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white focus:outline-none focus:border-[color:var(--color-turkish-blue-500)]/50"
                                                        >
                                                            {typeOptions.map(opt => (
                                                                <option key={opt.value} value={opt.value}>{opt.label}</option>
                                ...
                                <div className="space-y-6">
                                    <div className="p-6 rounded-3xl bg-white/5 border border-white/5 backdrop-blur-xl">
                                        ...
                                        <Reorder.Group axis="y" values={editData.screenshots || []} onReorder={(v) => setEditData(prev => ({ ...prev, screenshots: v }))} className="space-y-3">
                                            {(editData.screenshots || []).map((ss, idx) => (
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={e => {
                                                                ...
                                                                if (idx >= 0) {
                                                                    ...
                                                                    // @ts-expect-error Pushing to partial link type
                                                                    links.push({ type: linkDef.type, url: e.target.value });
                                                                }
