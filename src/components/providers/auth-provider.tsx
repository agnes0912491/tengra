...
export default function AuthProvider({
  ...
  const login = useCallback(async (email: string, password: string, type: "user" | "admin" = "user") => {
    ...
    try {
      ...
      const token = authenticated.authToken.token;
      if (!token) {
        return { success: false, reason: "unexpectedError" } as const;
      }
      localStorage.setItem("authToken", token);
      if (authenticated.authToken.refreshToken) localStorage.setItem("refreshToken", authenticated.authToken.refreshToken);
      if (authenticated.authToken.csrfToken) localStorage.setItem("csrfToken", authenticated.authToken.csrfToken);
